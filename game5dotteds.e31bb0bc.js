...
var rowsNum = 20;
var colsNum = 30;
var gamers = ['gamer1', 'gamer2']; //количество игроков

var gamerNum = 0; //кто начинает ходить первый
// заполняет игровое поле ячейками + возвращает кол-во ячеек
...
} // подсветка ячеек при клике


function cellClickHandler(e) {
  this.classList.add(gamers[gamerNum]);
  this.removeEventListener('click', cellClickHandler);
  isWin(gamers, lines); //если номер игрока > длины массива -> сбрасываем номер до 0
...
function getColumns(rows) {
  ...
  for (var i = 0; i < rows.length; i++) {
    if (columns[i] === undefined) columns[i] = []; //создание подмассив [ [] ]

    for (var j = 0; j < rows.length; j++) {
      columns[i][j] = rows[j][i]; //заполнение подмассива [ [1,4,6...] ]
    }
  }
